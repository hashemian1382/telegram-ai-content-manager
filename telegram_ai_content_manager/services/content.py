...
def scrape_channel(username, limit=10, timeout=20):
    ...
    response = httpx.get(
        f"https://t.me/s/{username}", headers=headers, timeout=timeout, follow_redirects=True
    )
...
def run_scrape(limit):
        ...
        try:
            ...
            for item in posts:
                ...
                if post is None:
                    ...
                    post.url, post.text, post.published_at, post.has_media, post.scraped_at = (
                        item["url"],
                        item["text"],
                        item["published_at"],
                        item["has_media"],
    ...
    run.completed_at, run.posts_found, run.posts_new, run.errors = (
        utcnow(),
        found,
        inserted,
        "\n".join(errors) or None,
...
def publish_draft(draft):
    ...
    request = Request(
        ...
        data=json.dumps({"chat_id": channel, "text": draft.text}).encode(),
    ...
    draft.status, draft.telegram_message_id, draft.published_at = (
        "published",
...
def generate_with_gemini(topic, model, source_post_ids=None, tone="professional", length="medium"):
    ...
    }.get(length, "180 to 300 Persian words")
    sources = []
