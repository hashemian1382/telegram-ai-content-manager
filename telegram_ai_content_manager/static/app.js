...
async function load() {
  try {
    ...
    $("#stats").innerHTML = [
      ...
      ["پیش‌نویس آماده", data.drafts],
      ["منتشرشده", data.published],
    ]
    ...
    $("#channels").innerHTML = channels.length
      ? channels
          .map(
            (c) =>
              `<div><a target="_blank" href="https://t.me/${c.username}">@${c.username}</a><button class="delete" onclick="removeChannel(${c.id})">حذف</button></div>`,
          )
          .join("")
    ...
    $("#drafts").innerHTML = drafts.length
      ? drafts
          .map(
            (d) =>
              `<div><div><b>${d.status === "published" ? "منتشر شد" : "پیش‌نویس"}</b><br><small>${esc(d.text.slice(0, 130))}${d.text.length > 130 ? "…" : ""}</small></div><button class="secondary" onclick="openDraft(${d.id})">مشاهده</button></div>`,
          )
          .join("")
...
async function addChannel(e) {
  ...
  try {
    await api("/api/channels", {
      method: "POST",
      body: JSON.stringify({ username: $("#channel").value }),
    });
    $("#channel").value = "";
...
async function publishDraft() {
  ...
  try {
    await saveDraft();
    activeDraft = await api("/api/drafts/" + activeDraft.id + "/publish", {
      method: "POST",
    });
    renderComposer();
...
async function showMode(mode) {
  const c = $("#creation");
  if (mode === "direct") {
    c.innerHTML = `<div class="ai-form"><label>متن پست</label><textarea id="directText" placeholder="متن پست خود را بنویسید…"></textarea><div class="create-actions"><button onclick="createDirect()">ساخت پیش‌نویس</button></div></div>`;
    return;
  }
  if (!sourcePosts.length) {
    try {
      sourcePosts = await api("/api/source-posts");
    } catch (e) {
      toast(e.message);
    }
  }
...
async function createDirect() {
  ...
  try {
    activeDraft = await api("/api/drafts/direct", {
      method: "POST",
      body: JSON.stringify({ text: $("#directText").value }),
    });
    $("#creation").innerHTML = "";
    renderComposer();
